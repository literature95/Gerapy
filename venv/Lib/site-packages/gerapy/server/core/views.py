...
def visualization_data(request):
    ...
    try:
        ...
        # 执行 SQL 查询
        mycursor.execute("SELECT title, star FROM movies")
        ...
        values = []
        for row in rows:
            labels.append(row[0])  # 电影名称
            values.append(row[1])  # 电影评分
...
def visualization(request):
    try:
        # 直接调用 visualization_data 函数获取数据
        data = visualization_data(request).data
    except Exception as e:
        data = {"error": str(e)}
...
def data_list(request):
    ...
    try:
        ...
        # 查询数据
        data_sql = f"""
            SELECT id, title as name, star as score, description, created_at 
            FROM movies 
            {where_clause}
            ORDER BY id DESC
            LIMIT %s OFFSET %s
        ...
        return JsonResponse({"message": f"获取数据列表失败: {str(e)}"}, 
                          status=500)
...
def data_detail(request, data_id):
    ...
    try:
        ...
        sql = """
            SELECT id, title as name, star as score, description, created_at,
                   director, scriptwriter, region, language
            FROM movies 
            WHERE id = %s
        """
        ...
        if not data:
            return JsonResponse({"message": "数据不存在"}, status=404)
        ...
        chart_sql = """
            SELECT 
                CASE 
                    WHEN star >= 9 THEN '9分以上'
                    WHEN star >= 8 THEN '8-9分'
                    WHEN star >= 7 THEN '7-8分'
                    WHEN star >= 6 THEN '6-7分'
                    ELSE '6分以下'
                END as category,
                COUNT(*) as value
            FROM movies
            GROUP BY category
            ORDER BY CASE 
                WHEN category = '9分以上' THEN 1
                WHEN category = '8-9分' THEN 2
                WHEN category = '7-8分' THEN 3
                WHEN category = '6-7分' THEN 4
                ELSE 5
            END
        """
        ...
        return JsonResponse({"message": f"获取数据详情失败: {str(e)}"}, 
                          status=500)
